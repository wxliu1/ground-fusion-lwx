...
#include <sophus/se3.hpp> // modified by wxliu on 2024-4-22
...
bool MotionEstimator::solveRelativeRT_PNP(const vector<pair<Vector3d, Vector3d>> &corres, Matrix3d &Rotation, Vector3d &Translation)
{
    ...
    // Matrix3d rota = SO3(rvec.at<double>(0, 0), rvec.at<double>(1, 0), rvec.at<double>(2, 0)).matrix(); // comment by wxliu on 2024-4-22.
    /*
     *  method 1 : ??? To be verified 
    Sophus::SO3d SO3_obj = Sophus::SO3d::exp(Eigen::Vector3d(rvec.at<double>(0, 0), rvec.at<double>(1, 0), rvec.at<double>(2, 0))); // modified by wxliu on 2024-4-22.
    Matrix3d rota = SO3_obj.matrix();
    */

    // [2024-4-22] method2: 此处旋转向量经罗德里格斯转换
    cv::Mat R;
    cv::Rodrigues(rvec, R);
    Eigen::Matrix3d rota;
    rota << R.at<double>(0,0), R.at<double>(0,1), R.at<double>(0,2),
